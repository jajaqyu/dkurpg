...
func _on_time_up():
	if isCheck:
		...
		game_over_dialog.dialog_text = "몬스터 처치 수: %d, 오르는 스텟: %s, 오르는 양: %d"%[monsters_defeated,"DEF",tmp]
...
func stat_update():
	...
	db.query("UPDATE character SET DEF = DEF + %d, Progress = %d,HP = %d, ItemCount = %d WHERE character_name = '%s'" % [tmp, HUD.progress,HUD.progress/2,HUD.itemCount,HUD.char_name])
	db.close_db()
